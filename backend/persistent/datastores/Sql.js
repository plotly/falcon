...
export function schemas(connection) {
    ...
    switch (dialect) {
        ...
        case DIALECTS.MARIADB:
            queryString = `SELECT table_name, column_name, data_type FROM information_schema.columns WHERE table_schema = '${database}' ORDER BY table_name`;
            break;
        case DIALECTS.POSTGRES:
            queryString = `SELECT table_name, column_name, data_type FROM information_schema.columns WHERE table_catalog = '${database}' AND table_schema = 'public' ORDER BY table_name`;
            break;
        case DIALECTS.REDSHIFT:
            queryString = `SELECT tablename, "column", "type" FROM pg_table_def WHERE schemaname = 'public';`
            break;
        case DIALECTS.MSSQL:
            queryString = `SELECT T.name AS Table_Name, C.name AS Column_Name, P.name AS Data_Type, P.max_length AS Size, CAST(P.precision AS VARCHAR) + '/' + CAST(P.scale AS VARCHAR) \
