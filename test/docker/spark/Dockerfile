...
RUN chmod +x /setup_java.sh ; /setup_java.sh
...
ENTRYPOINT /etc/bootstrap.sh && service mysql start && chmod +x /bin/setup_database.sh && /bin/setup_database.sh && /livy/bin/livy-server;
